...
const PaymentCOD = () => {
  ...
  // ...existing code for cartItems and useEffect should only appear once...
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const orderPayload = {
        ...
        payment_method: 'cod',
        currency: 'PHP',
        gcash_details: null // or '' for COD, can be object/string for online payment
      };
      ...
      setTimeout(() => navigate('/foodmenu'), 2000);
    } catch (err) {
      ...
      <div className="flex items-center justify-center py-10 px-2">
        <div className="bg-white rounded-xl shadow-2xl flex flex-col md:flex-row w-full max-w-4xl overflow-hidden mt-32">
          ...
          <form onSubmit={handleSubmit} className="md:w-1/2 w-full p-8 flex flex-col justify-center">
            <h2 className="text-2xl font-bold mb-6 text-gray-900 text-center md:text-left">Delivery Details</h2>
            {error && <div className="mb-4 text-red-600 text-sm">{error}</div>}
            <div className="mb-4">
              <label className="block mb-1 font-medium">Delivery Address<span className="text-red-500">*</span></label>
              <textarea name="address" value={form.address} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" rows={2} />
            </div>
            <div className="mb-4">
              <label className="block mb-1 font-medium">Contact Number<span className="text-red-500">*</span></label>
              <input type="text" name="contact" value={form.contact} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>
            <div className="mb-4">
              <label className="block mb-1 font-medium">Customer Note (Optional)</label>
              <textarea name="note" value={form.note} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" rows={2} />
            </div>
            <button type="submit" className="w-full bg-gradient-to-r from-green-600 to-emerald-700 text-white py-3 rounded-lg font-bold text-lg hover:from-emerald-700 hover:to-green-900 transition shadow-xl disabled:opacity-60" disabled={loading}>
              {loading ? 'Placing Order...' : 'Place COD Order'}
            </button>
            {success && <div className="mt-4 text-green-600 text-center font-semibold">Order placed successfully!</div>}
          </form>
        </div>
      </div>
    </div>
